...
internal struct StickyNoteCheck: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.625*width, y: 0.7916666667*height))
        ...
        strokePath6.addLine(to: CGPoint(x: 0.4791666667*width, y: 0.875*height))
