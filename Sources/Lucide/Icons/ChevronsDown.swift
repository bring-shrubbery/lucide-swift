...
internal struct ChevronsDown: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.2916666667*width, y: 0.25*height))
        ...
        strokePath4.move(to: CGPoint(x: 0.2916666667*width, y: 0.5416666667*height))
