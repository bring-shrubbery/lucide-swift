...
internal struct SquareArrowOutUpRight: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.875*width, y: 0.5416666667*height))
        ...
        strokePath2.addLine(to: CGPoint(x: 0.4583333333*width, y: 0.125*height))
