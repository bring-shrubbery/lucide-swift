...
internal struct MonitorX: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.6041666667*width, y: 0.5208333333*height))
        strokePath2.addLine(to: CGPoint(x: 0.3958333333*width, y: 0.3125*height))
        path.addPath(strokePath2.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath4 = Path()
        strokePath4.move(to: CGPoint(x: 0.3958333333*width, y: 0.5208333333*height))
        strokePath4.addLine(to: CGPoint(x: 0.6041666667*width, y: 0.3125*height))
