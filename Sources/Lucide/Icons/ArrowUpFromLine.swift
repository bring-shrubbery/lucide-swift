...
internal struct ArrowUpFromLine: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.75*width, y: 0.375*height))
        strokePath2.addLine(to: CGPoint(x: 0.5*width, y: 0.125*height))
        strokePath2.addLine(to: CGPoint(x: 0.25*width, y: 0.375*height))
