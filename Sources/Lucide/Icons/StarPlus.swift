...
internal struct StarPlus: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.458875*width, y: 0.77425*height))
        ...
        strokePath2.addLine(to: CGPoint(x: 0.8333333333*width, y: 0.4791666667*height))
