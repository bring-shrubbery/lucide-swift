...
internal struct FolderSearch: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.4458333333*width, y: 0.8333333333*height))
        ...
        strokePath2.addLine(to: CGPoint(x: 0.9166666667*width, y: 0.5041666667*height))
        path.addPath(strokePath2.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath4 = Path()
        strokePath4.move(to: CGPoint(x: 0.875*width, y: 0.875*height))
        strokePath4.addLine(to: CGPoint(x: 0.7958333333*width, y: 0.7958333333*height))
