...
internal struct UserX: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath4.move(to: CGPoint(x: 0.7083333333*width, y: 0.3333333333*height))
        strokePath4.addLine(to: CGPoint(x: 0.9166666667*width, y: 0.5416666667*height))
        path.addPath(strokePath4.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath5 = Path()
        strokePath5.move(to: CGPoint(x: 0.9166666667*width, y: 0.3333333333*height))
