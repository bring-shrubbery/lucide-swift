...
internal struct Venus: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath4.move(to: CGPoint(x: 0.375*width, y: 0.7916666667*height))
        strokePath4.addLine(to: CGPoint(x: 0.625*width, y: 0.7916666667*height))
        path.addPath(strokePath4.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath5 = Path()
        strokePath5.addEllipse(in: CGRect(x: 0.25*width, y: 0.125*height, width: 0.5*width, height: 0.5*height))
