...
internal struct FaceSlightlyFrowning: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath4.addLine(to: CGPoint(x: 0.375*width, y: 0.375*height))
        path.addPath(strokePath4.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath6 = Path()
        strokePath6.move(to: CGPoint(x: 0.375*width, y: 0.6666666667*height))
        strokePath6.addCurve(to: CGPoint(x: 0.625*width, y: 0.6666666667*height), control1: CGPoint(x: 0.4490740741*width, y: 0.6111111111*height), control2: CGPoint(x: 0.5509259259*width, y: 0.6111111111*height))
