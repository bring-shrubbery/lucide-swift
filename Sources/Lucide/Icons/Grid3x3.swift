...
internal struct Grid3x3: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath3.addLine(to: CGPoint(x: 0.875*width, y: 0.375*height))
        path.addPath(strokePath3.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath5 = Path()
        strokePath5.move(to: CGPoint(x: 0.125*width, y: 0.625*height))
        ...
        strokePath7.addLine(to: CGPoint(x: 0.375*width, y: 0.875*height))
        path.addPath(strokePath7.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath9 = Path()
        strokePath9.move(to: CGPoint(x: 0.625*width, y: 0.125*height))
        strokePath9.addLine(to: CGPoint(x: 0.625*width, y: 0.875*height))
