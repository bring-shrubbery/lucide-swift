...
internal struct ChevronsLeft: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath2.addLine(to: CGPoint(x: 0.4583333333*width, y: 0.2916666667*height))
        path.addPath(strokePath2.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath4 = Path()
        strokePath4.move(to: CGPoint(x: 0.75*width, y: 0.7083333333*height))
        strokePath4.addLine(to: CGPoint(x: 0.5416666667*width, y: 0.5*height))
