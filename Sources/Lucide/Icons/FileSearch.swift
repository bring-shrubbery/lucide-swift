...
internal struct FileSearch: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath5.addEllipse(in: CGRect(x: 0.375*width, y: 0.5*height, width: 0.2083333333*width, height: 0.2083333333*height))
        path.addPath(strokePath5.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath7 = Path()
        strokePath7.move(to: CGPoint(x: 0.5541666667*width, y: 0.6791666667*height))
        strokePath7.addLine(to: CGPoint(x: 0.625*width, y: 0.75*height))
