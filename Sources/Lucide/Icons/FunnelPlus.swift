...
internal struct FunnelPlus: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.5564166667*width, y: 0.125*height))
        ...
        strokePath2.addLine(to: CGPoint(x: 0.655625*width, y: 0.4712916667*height))
        ...
        strokePath6.move(to: CGPoint(x: 0.7916666667*width, y: 0.125*height))
