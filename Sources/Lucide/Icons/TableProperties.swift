...
internal struct TableProperties: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath5.move(to: CGPoint(x: 0.875*width, y: 0.375*height))
        strokePath5.addLine(to: CGPoint(x: 0.125*width, y: 0.375*height))
        path.addPath(strokePath5.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath7 = Path()
        strokePath7.move(to: CGPoint(x: 0.875*width, y: 0.625*height))
        strokePath7.addLine(to: CGPoint(x: 0.125*width, y: 0.625*height))
