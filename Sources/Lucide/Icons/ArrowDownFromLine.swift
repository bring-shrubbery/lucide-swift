...
internal struct ArrowDownFromLine: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        var strokePath2 = Path()
        strokePath2.move(to: CGPoint(x: 0.7916666667*width, y: 0.125*height))
        ...
        strokePath6.move(to: CGPoint(x: 0.25*width, y: 0.625*height))
