...
internal struct Dice2: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        strokePath3.addLine(to: CGPoint(x: 0.6254166667*width, y: 0.375*height))
        path.addPath(strokePath3.strokedPath(StrokeStyle(lineWidth: 0.0833333333*width, lineCap: .round, lineJoin: .round, miterLimit: 4)))
        var strokePath5 = Path()
        strokePath5.move(to: CGPoint(x: 0.375*width, y: 0.625*height))
        strokePath5.addLine(to: CGPoint(x: 0.3754166667*width, y: 0.625*height))
